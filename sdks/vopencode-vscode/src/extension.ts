...
class AppController implements vscode.Disposable {
  ...
  private readonly output = vscode.window.createOutputChannel("OpenCode App")
    ...
    for (let i = 0; i < next.length; i++) {
      ...
      if (cur === "--port" || cur === "-p") {
        ...
        if (!val || val.startsWith("-")) {
          next.splice(i + 1, 0, "4444")
          i++
        }
        continue
      }
      if (cur.startsWith("--port=")) {
        const val = cur.slice("--port=".length)
        if (!val) {
          next[i] = "--port=4444"
        }
      }
  ...
  private config(): AppConfig {
    ...
    const raw = cfg.get<string[]>("app.args", ["dev", "--", "--port", "4444"])
    const args = this.args(raw)
    return {
  ...
  private resolveCommand(command: string) {
    ...
    if (fs.existsSync(fnmScoopPath)) {
      return fnmScoopPath
    }
    // fnm installed in AppData
    const fnmAppDataPath = path.join(
    ...
    if (fs.existsSync(fnmAppDataPath)) {
      return fnmAppDataPath
    }
    return command
  }
  ...
  async resolve(baseServerUrl: string) {
    ...
    if (!repo) {
      throw new Error("Cannot resolve repo root for app startup. Set opencode.app.sourceRepoPath.")
    }
...
class ServiceController implements vscode.Disposable {
  ...
  constructor() {
    ...
    this.status.tooltip = "Open OpenCode"
    this.status.show()
  ...
  private findRepoRoot(start: string) {
    ...
    const first = fs.existsSync(start) ? fs.realpathSync(start) : start
    const chain: string[] = []
    let cur = first
    for (let i = 0; i < 8; i++) {
      chain.push(cur)
  ...
  async ensureRunning() {
    ...
    if (config.mode === "manual") {
      this.setState("stopped", `OpenCode server is not running at ${this.url}`)
      throw new Error(`OpenCode server is not running at ${this.url}`)
    }
    ...
    // Wait for any ongoing start operation
  ...
  private async start(config: ReturnType<ServiceController["getConfig"]>) {
    ...
    if (!cwd) {
      throw new Error(
        "Cannot resolve source repository path. Set opencode.server.sourceRepoPath to your cloned opencode repo root.",
      )
    }
    if (!fs.existsSync(cwd)) {
    ...
    this.output.appendLine(`Starting OpenCode server in ${cwd}`)
    const command = this.resolveCommand(config.command)
    this.output.appendLine(`> ${command} ${config.args.join(" ")}`)
    ...
    this.proc.on("error", (error) => {
      ...
      this.output.appendLine(`OpenCode server failed to spawn: ${this.lastSpawnError}`)
      this.proc = undefined
      this.setState("error", this.lastSpawnError)
    })

    this.proc.on("exit", (code, signal) => {
      this.output.appendLine(`OpenCode server exited (code=${code}, signal=${signal})`)
      this.proc = undefined
    ...
    while (Date.now() - start < config.startTimeoutMs) {
      if (this.lastSpawnError) {
        throw new Error(`Failed to start OpenCode server: ${this.lastSpawnError}`)
      }
      if (!this.proc) {
        if (this.preloadError) {
          throw new Error(
            "OpenCode dependencies are missing. Run 'bun install' at the opencode repo root, then restart extension host.",
          )
        }
        throw new Error("OpenCode server exited before becoming healthy. Check OpenCode output channel.")
      }
      if (await this.alive(this.url)) {
        this.setState("running")
        return
    ...
    this.setState("error", `OpenCode server did not become healthy within ${config.startTimeoutMs}ms`)
    throw new Error(`OpenCode server did not become healthy within ${config.startTimeoutMs}ms. Check OpenCode output.`)
  }
...
<body>
  <div class="wrap">
    <div class="hero">
      ...
      <div class="grid">
        <button id="open" class="icon-btn" title="打开opencode" aria-label="打开opencode">◉</button>
        <button id="new" class="icon-btn" title="新建opencode" aria-label="新建opencode">⊕</button>
        <button id="copy" class="icon-btn" title="添加当前文件到上下文" aria-label="添加当前文件到上下文">⧉</button>
        <button id="closeAll" class="icon-btn" title="关闭所有opencode" aria-label="关闭所有opencode">✕</button>
...
async function openChat(
  ...
  const next = vscode.window.createWebviewPanel("opencode.chat", "OpenCode", vscode.ViewColumn.Beside, {
    enableScripts: true,
...
function loadingHtml() {
  return '<!doctype html><html><body style="font-family: var(--vscode-font-family); padding:16px;">Starting OpenCode...</body></html>'
}

function errorHtml(message: string) {
  return `<!doctype html><html><body style="font-family: var(--vscode-font-family); padding:16px;"><h3>OpenCode failed to start</h3><pre>${escapeHtml(message)}</pre></body></html>`
}
...
<html>
  <body style="margin:0; padding:0; overflow:hidden; background: var(--vscode-editor-background);">
    ...
    <iframe id="opencode-iframe" title="OpenCode" src="${safeUrl}" style="border:0; width:100vw; height:100vh;"></iframe>
    <script nonce="${nonce}">
      const vscode = acquireVsCodeApi();
      window.addEventListener('message', (event) => {
