...
function partKind(part: PartType) {
  if (part.type === "reasoning") return "thinking"
  if (part.type === "tool") return "process"
  if (part.type === "text") return "answer"
  return "other"
}

function processSymbol(tool: string) {
  if (tool === "bash") return "$"
  if (tool === "read") return "#"
      ...
      {(key) => {
        ...
        return (
          <>
            <Show when={ctx()}>
              {(entry) => (
                <div data-component="assistant-block" data-kind="process" data-process="context">
                  <ContextToolGroup parts={entry().parts} busy={props.working && tail()} />
                </div>
              )}
            </Show>
            <Show when={part()}>
              {(entry) => {
                const kind = createMemo(() => partKind(entry().part))
                return (
                  <div data-component="assistant-block" data-kind={kind()}>
                    <Part
                      part={entry().part}
        ...
        return (
          <>
            <Show when={ctx()}>
              {(entry) => (
                <div data-component="assistant-block" data-kind="process" data-process="context">
                  <ContextToolGroup parts={entry().parts} />
                </div>
              )}
            </Show>
            <Show when={part()}>
  ...
  return (
    <Show when={!hideQuestion()}>
      <div
        ...
        style={{ "--process-symbol": `'${processSymbol(part.tool)}'` }}
      >
        <Switch>
  ...
  return (
    <Show when={throttledText()}>
      <div data-component="reasoning-part">
        <div data-slot="reasoning-label">Thinking:</div>
        <Markdown text={throttledText()} cacheKey={part.id} />
      </div>
    </Show>
  )
