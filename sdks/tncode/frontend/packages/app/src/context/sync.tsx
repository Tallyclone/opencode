...
import { readData, readList } from "@/utils/backend-compat"
import { useGlobalSync } from "./global-sync"
import { useSDK } from "./sdk"
import type { FileDiff, Message, Part, Session, Todo } from "@opencode-ai/sdk/v2/client"
      ...
      session: {
        ...
        async sync(sessionID: string) {
          ...
          const sessionReq = hasSession
            ...
            : retry(() => client.session.get({ sessionID })).then((session) => {
                const data = readData<Session>(session)
                if (!data) return
                setStore(
                  "session",
                  produce((draft) => {
        ...
        async diff(sessionID: string) {
          ...
          return runInflight(inflightDiff, key, () =>
            retry(() => client.session.diff({ sessionID })).then((diff) => {
              setStore("session_diff", sessionID, reconcile(readList<FileDiff>(diff), { key: "file" }))
            }),
          )
        },
        ...
        fetch: async (count = 10) => {
          ...
          await client.session.list().then((x) => {
            const sessions = readList<Session>(x)
              .filter((s) => !!s?.id)
              .sort((a, b) => cmp(a.id, b.id))
              .slice(0, store.limit)
