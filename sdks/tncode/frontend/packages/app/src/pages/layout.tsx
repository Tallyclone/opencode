...
import { readData, readList, readValue } from "@/utils/backend-compat"
...
import { LEFT_RAIL_WIDTH, SIDEBAR_MIN_WIDTH } from "./layout/dimensions"
...
export default function Layout(props: ParentProps) {
  ...
  const vscodeWorkspaceDirectory = createMemo(() => {
    if (typeof window === "undefined") return ""
    const query = new URLSearchParams(window.location.search)
    if (query.get("caller") !== "vscode") return ""
  ...
  createEffect(() => {
    if (state.vscodeBootstrapped) return
    if (!pageReady()) return
    if (!layoutReady()) return
    const directory = vscodeWorkspaceDirectory().trim()
    if (!directory) return
    setState("vscodeBootstrapped", true)
    openProject(directory, false)
    const project = layout.projects
      .list()
      .find((item) => item.worktree === directory || (item.sandboxes ?? []).includes(directory))
    const target = project?.worktree ?? directory
    layout.projects.move(target, 0)
  ...
  async function prefetchMessages(directory: string, sessionID: string, token: number) {
    ...
    return retry(() => globalSDK.client.session.messages({ directory, sessionID, limit: prefetchChunk }))
      .then((messages) => {
        ...
        const items = readList<{ info: Message; parts: Part[] }>(messages).filter((x) => !!x?.info?.id)
        ...
        batch(() => {
          ...
          for (const message of items) {
            ...
            const mergedParts = mergeByID(
              ...
              message.parts.filter((item): item is Part & { id: string } => !!item?.id),
            )
  ...
  createEffect(() => {
    const sidebarWidth = layout.sidebar.opened() ? layout.sidebar.width() : LEFT_RAIL_WIDTH
  ...
  const createWorkspace = async (project: LocalProject) => {
    ...
    const created = await globalSDK.client.worktree
      ...
      .then((x) => readData<{ directory?: string; branch?: string }>(x))
      .catch((err) => {
        showToast({
    ...
    setWorkspaceName(directory, branch, project.id, branch)
    ...
    return (
      ...
      >
        <Show when={panelProps.project}>
          {(p) => (
            <>
              <div class="shrink-0 px-0.5 py-0.5">
                <div class="group/project flex items-start justify-between gap-2 p-0.5 pr-0.5">
              ...
              <div class="flex-1 min-h-0 flex flex-col">
                <Show
                  ...
                  fallback={
                    <>
                      <div class="shrink-0 py-2 px-[3px]">
                        <TooltipKeybind
                    ...
                    <div class="relative flex-1 min-h-0">
                      ...
                      >
                        ...
                        <div
                          ...
                          class="size-full flex flex-col py-1 gap-4 overflow-y-auto no-scrollbar [overflow-anchor:none]"
                        >
        ...
        >
          <div class="rounded-md bg-background-base shadow-xs-border-base">
            ...
            <Button
              class="flex w-full text-left justify-start text-12-medium text-text-strong stroke-[1.5px] rounded-md rounded-t-none shadow-none border-t border-border-weak-base px-[3px]"
              size="large"
              icon="plus"
    ...
    <div class="relative bg-background-base flex-1 min-h-0 flex flex-col select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text">
      ...
      <div class="flex-1 min-h-0 flex">
        <nav
          ...
          style={{
            width: layout.sidebar.opened()
              ? `${Math.max(layout.sidebar.width(), SIDEBAR_MIN_WIDTH)}px`
              : `${LEFT_RAIL_WIDTH}px`,
          }}
          ref={(el) => {
            setState("nav", el)
        ...
        >
          ...
          <Show when={layout.sidebar.opened() && !fullLayout}>
            <ResizeHandle
        ...
        <div class={fullLayout ? "hidden" : "xl:hidden"}>
          <div
            classList={{
        ...
        <main
          classList={{
            ...
            "xl:border-l xl:rounded-tl-[12px]": !layout.sidebar.opened() && !fullLayout,
            "border-l rounded-tl-[12px]": !layout.sidebar.opened() && fullLayout,
            "rounded-tr-[12px]": fullLayout,
          }}
        >
