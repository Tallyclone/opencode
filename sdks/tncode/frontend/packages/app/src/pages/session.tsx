...
export default function Page() {
  ...
  const isDesktop = createMemo(() => true)
  ...
  const insertRefInPrompt = (ref: string) => {
    const value = ref.trim()
    if (!value.startsWith("@")) return
    const raw = value.slice(1)
    const [path, linePart] = raw.split("#")
    if (!path) return
    const selection = (() => {
      if (!linePart) return undefined
      const single = linePart.match(/^L(\d+)$/)
      if (single) {
      ...
      const range = linePart.match(/^L(\d+)-(\d+)$/)
  ...
  createEffect(() => {
    ...
    const listener = (event: MessageEvent) => {
      if (!event.data || event.data.type !== "opencode.insertRef") return
      if (typeof event.data.ref !== "string") return
      insertRefInPrompt(event.data.ref)
    }
  ...
  return (
    <div class="relative bg-background-base size-full overflow-hidden flex flex-col">
      <SessionHeader />
      <div class="flex-1 min-h-0 relative session-shell">
        <div class="size-full flex flex-row" style={{ "padding-right": `${RIGHT_RAIL_WIDTH}px` }}>
          {/* Session panel */}
          <div
            classList={{
              "@container relative min-w-0 flex flex-col min-h-0 h-full bg-background-stronger session-content-surface": true,
              "flex-1": true,
          ...
          >
            <div class="flex-1 min-h-0 overflow-hidden flex">
              ...
              <div id="opencode-session-rail" class="shrink-0" />
            </div>
          </div>
        ...
        <div
          data-component="session-right-rail"
          class="absolute inset-y-0 right-0 session-right-rail flex items-start justify-center pt-2 transition-colors duration-[160ms] ease-out"
          style={{ width: `${RIGHT_RAIL_WIDTH}px` }}
        >
          <div class="flex flex-col gap-2 w-full items-center">
            <div class="session-right-rail-group">
              <TooltipKeybind title={language.t("session.header.searchFiles")} keybind={command.keybind("file.open")}>
                <Button
                  variant="ghost"
                  icon="magnifying-glass"
                  class="titlebar-icon session-rail-action w-6 h-6 p-0 box-border"
                  onClick={() => command.trigger("file.open")}
                  aria-label={language.t("session.header.searchFiles")}
                />
              </TooltipKeybind>
              <Tooltip value={language.t("session.header.open.copyPath")} placement="left" gutter={8}>
                <Button
                  variant="ghost"
                  icon="copy"
                  class="titlebar-icon session-rail-action w-6 h-6 p-0 box-border"
              ...
              <Tooltip value={language.t("session.share.action.share")} placement="left" gutter={8}>
                <Button
                  variant="ghost"
                  icon="share"
                  class="titlebar-icon session-rail-action w-6 h-6 p-0 box-border"
                  onClick={() => command.trigger("session.share")}
                  aria-label={language.t("session.share.action.share")}
                />
              </Tooltip>
            </div>
            <div class="session-right-rail-divider" />
            <div class="session-right-rail-group">
              <TooltipKeybind
              ...
              <TooltipKeybind title={language.t("command.review.toggle")} keybind={command.keybind("review.toggle")}>
                <Button
                  ...
                  icon="layout-right"
                  class="titlebar-icon session-rail-action w-6 h-6 p-0 box-border"
                  classList={{ "session-rail-action-active": view().reviewPanel.opened() }}
              ...
              >
                <Button
                  ...
                  class="titlebar-icon session-rail-action w-6 h-6 p-0 box-border"
                  classList={{ "session-rail-action-active": layout.fileTree.opened() }}
