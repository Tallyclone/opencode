...
import { readValue } from "@/utils/backend-compat"
...
import { SESSION_HEADER_HEIGHT, SESSION_STICKY_ACCORDION_TOP } from "@/pages/layout/dimensions"
...
export function MessageTimeline(props: {
  ...
  const caller = createMemo(() => {
    if (typeof window === "undefined") return ""
    return new URLSearchParams(window.location.search).get("caller") ?? ""
  })
  const vscode = createMemo(() => caller() === "vscode")
    ...
    >
      <div class="relative w-full h-full min-w-0">
        ...
        <div
          ...
          style={{
            "--session-title-height": showHeader() ? `${SESSION_HEADER_HEIGHT}px` : "0px",
            "--sticky-accordion-top": showHeader() ? `${SESSION_STICKY_ACCORDION_TOP}px` : "0px",
          }}
        >
          <Show when={showHeader()}>
            <div
              classList={{
                "sticky top-0 z-30 session-thread-header-shell": true,
                "w-full": true,
                "pb-4": true,
            ...
            >
              <div class="h-12 w-full flex items-center justify-between gap-2 session-thread-header-card">
          ...
          <div
            ...
            role="log"
            class="flex flex-col gap-12 items-start justify-start pb-16 pt-2 md:pt-3 transition-[margin] session-thread-content"
            classList={{
              "w-full": true,
              "md:max-w-200 md:mx-auto 2xl:max-w-[1000px]": props.centered,
