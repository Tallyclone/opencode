...
export function TerminalPanel() {
  ...
  const mediaDesktop = createMediaQuery("(min-width: 768px)")
  const isDesktop = createMemo(() => forceDesktopLayout() || mediaDesktop())
  const compactDesktop = createMemo(
    () => forceDesktopLayout() && typeof window !== "undefined" && window.innerWidth < 1160,
  )
    ...
    <Show when={rendered()}>
      <div
        ...
        classList={{
          ...
          "border-t border-border-weak-base": open() && !compactDesktop(),
          "pointer-events-none": !open(),
          "fixed inset-x-0 bottom-0 z-40 bg-background-stronger shadow-2xl": compactDesktop(),
        }}
        style={{
          height: compactDesktop() ? "min(56vh, 560px)" : `${height()}px`,
          display: open() ? "flex" : "none",
        }}
      >
        <Show when={compactDesktop()}>
          <div
            class="fixed inset-0 z-[-1] bg-black/40"
            onClick={(event) => {
