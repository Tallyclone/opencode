...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const runningMarquee = createMemo(() => {
    if (!vscode()) return ""
    const current = status()
    if (current?.type === "retry") return "tncode is retrying and still working"
    if (current?.type === "busy") return "tncode is still working"
    return ""
  })
  ...
  const modelLabel = createMemo(() => {
    ...
    if (!current) return language.t("dialog.model.select.title")
    const provider = current.provider?.name || current.provider?.id
    if (!provider) return current.name
    return `${provider} / ${current.name}`
  })
  ...
  return (
    <div class="relative size-full _max-h-[320px] flex flex-col gap-0">
      ...
      <Show when={runningMarquee()}>
        <div class="prompt-running-marquee" aria-live="polite">
          <div class="prompt-running-marquee-track">
            <span>{runningMarquee()}</span>
            <span>{runningMarquee()}</span>
            <span>{runningMarquee()}</span>
          </div>
        </div>
